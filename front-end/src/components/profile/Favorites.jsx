...
export default function Favorites() {
  ...
  const removeFromFavorites = async (lockId) => {
    try {
      await api.delete(`/api/favorites/${lockId}`); 
  ...
  if (favorites.length === 0) {
    return (
      <div className="text-center py-20 text-gray-500 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
        ...
        <p className="text-xl mb-4">Вы ещё ничего не добавили в избранное</p>
    ...
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {favorites.map((lock) => (
          <div
            key={lock.id}
            className="bg-white rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all group border border-gray-100"
          >
            ...
            <div className="p-6">
              <Link to={`/product/${lock.id}`} className="block">
                <h3 className="font-bold text-xl text-gray-900 hover:text-indigo-600 transition line-clamp-2 h-14">
                  {lock.name}
                </h3>
                <p className="text-gray-500 mt-2 text-sm">Артикул: {lock.article}</p>
              </Link>
                ...
                <div>
                  ...
                  {lock.price_with_discount && (
                    <del className="text-sm text-gray-400">
                      {Number(lock.price).toLocaleString()} ₽
                    </del>
                  )}
                </div>
                <button className="bg-indigo-600 text-white p-3 rounded-2xl hover:bg-indigo-700 transition shadow-lg hover:shadow-indigo-500/30">
